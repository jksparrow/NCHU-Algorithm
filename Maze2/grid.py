...
class Grid:
    ...
    def valid_ganeighbors(self, cell):
        i = cell.i
        j = cell.j
        ganeighbors = []
        # Top neighbor
        if cell.i > 0 and cell.lines['top'][0] == False:
            ganeighbors.append((0,self.grid[i - 1][j]))
        # Bottom neighbor
        if cell.i < self.num_rows - 1 and cell.lines['bottom'][0] == False:
            ganeighbors.append((1,self.grid[i + 1][j]))
        # Right neighbor
        if cell.j < self.num_cols - 1 and cell.lines['right'][0] == False:
            ganeighbors.append((2,self.grid[i][j + 1]))
        # Left neighbor
        if cell.j > 0 and cell.lines['left'][0] == False:
            ganeighbors.append((3,self.grid[i][j - 1]))
        return ganeighbors
